...
import { changeUserName, logoutAC, meTC, changeSubscribe } from '../../../BLL/AuthReducer';


const Cabinet = () => {
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  const [selectedSubscribe, setSelectedSubscribe] = useState({value: "", label: ""});
  /**
  ...
  /**
    ...
    const handleSelectSubscribe = (selectedSubscribe : any) : void => {
      console.log(selectedSubscribe)
      setSelectedSubscribe(selectedSubscribe);
    };

    const handleChooseSubscribe = () : void => {
      const token  = localStorage.getItem('jwtToken') as string;
      console.log(selectedSubscribe)
      console.log(token)
      let changeSubscribeElems = {
        "user": {
          "code": token
        },
        "type": selectedSubscribe.value
      }
      dispatch(changeSubscribe(changeSubscribeElems))
    };
  ...
  const options = [
    ...
    { value: 'MTH', label: 'Месячная подписка' },
    { value: '6MTH', label: '6-месячная подписка' },
  ];
               ...
               <div className={style.wrapper}>
                <div className={style.contentFlexBox}>
                 <div className={style.personalData}>
                   ...
                   <input type="email" placeholder={profileData?.email} disabled/>
                 ...
                 <div className={style.rateData}>
                   ...
                   onChange={handleSelectSubscribe} 
                   />
                   <button className={style.btn} onClick={handleChooseSubscribe}>
                     ИЗМЕНИТЬ ТАРИФ
                   </button>
